...
const convert = __importStar(__nccwpck_require__(821));
const fs = __importStar(__nccwpck_require__(147));
const path = __importStar(__nccwpck_require__(17));
const config_1 = __importDefault(__nccwpck_require__(88));
const Permissions = __importStar(__nccwpck_require__(468));
class Options {
    constructor() {
...
function mergeObjectPermission(array) {
    const local = [];
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.merge = exports.ObjectPermission = void 0;
...
function isObjectPermission(obj) {
    if (obj._attributes['android:name']) {
        return true;
    }
    return false;
}
function isArrayOfObjectPermission(arr) {
    ...
    for (const arg of args) {
        if (arg) {
            if (Array.isArray(arg)) {
                if (isArrayOfObjectPermission(arg))
                    result = [...result, ...arg];
            }
            else {
                if (isObjectPermission(arg))
                    result.push(arg);
            }
        }
    }
...
function replaceInFile(filename, key, value, valueType) {
    switch (valueType) {
        case 'number':
            if (isNaN(+value))
...
const AndroidManifest = __importStar(__nccwpck_require__(363));
